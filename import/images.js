...
function resizeImagesToMatch(images) {
	var images = images.filter(function (item) {
		item.type !== "image/svg+xml" ? true : false; 
	});
	return Promises.all(images.map(function (img) {
			var sizes = {};
			if (img.width) {
				sizes.width = Math.min(Number.parseInt(img.width) * 2, 2000);
			} else if (img.height) {
				sizes.height = Math.min(Number.parseInt(img.height) * 2, 2000);
			}
			if (sizes) {
				...
				return gd.openAsync(image).then(function (image) {
